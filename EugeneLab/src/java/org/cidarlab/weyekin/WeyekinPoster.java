...
//            httpPost.setURI(new URI("http://128.197.164.27/graphviz/graphviz1.php"));
    ...
    public static String postMyBird() {
        DefaultHttpClient httpclient = new DefaultHttpClient();
...
//            httpPost.setURI(new URI("http://128.197.164.27/pigeon.php"));
        ...
        if (!mPigeonString.isEmpty()) {
            return parsePigeonResponse();
        }
        return (String)null;
    }
    
    private static String parsePigeonResponse() {
        ...
        for (String s : split) {
            if (s.contains(mPigeonImageIdentifier)) {
               ...
               try {
                    return new URI(path).toURL().toString();
               } catch(Exception e) {
                    return null;
               }
